...
import { Smile, CalendarClock,LocationEdit, FlagTriangleRight, GalleryThumbnails } from "lucide-react";
  ...
  return (
    <div className="fixed inset-0  bg-black/70 flex items-center justify-center z-50 ">
        <div className="absolute top-15 left-[33.6vw] bg-black w-[600px] min-h-[350px]  rounded-2xl">
            <button className="cursor-pointer p-2 rounded-full hover:bg-gray-900 mb-4 mx-2 mt-2" onClick={()=>setShowReply(false)} ><X size={20}/></button>
      ...
      <div className="flex w-fit items-start gap-2 mx-4 my-2 cursor-pointer">
        ...
        <div className="flex flex-col">
          ...
          <div className=" mx-16 text-gray-600">Replying to <span className="text-blue-400">@{post.user.username}</span></div>
          
          <div className="flex mx-4 my-2 gap-2 ">
            
      <img className="rounded-full object-cover w-10 h-10" src={`${user.profilePic}` || "/default-avatar.png"} />
        <textarea rows={1} onInput={(e) => {
    e.target.style.height = "auto"
    e.target.style.height = e.target.scrollHeight + "px";
  }}className="outline-none resize-none placeholder:text-xl min-h-24 text-xl w-full" onChange={(e)=> setPostText(e.target.value)} value={postText} type="text" placeholder="Post your reply"/>
          </div>
      </div>
